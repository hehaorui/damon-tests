...
def parse_timestamped_file(input_file, output_file):
    """
    Parse timestamped SwapCached data file and convert to CSV format.
    ...
    try:
        with open(input_file, 'r') as f:
            for line in f:
                ...
                # Parse line format: [timestamp_ns] SwapCached: value unit
    ...
    try:
        with open(output_file, 'w') as f:
            # Write CSV header
            f.write("timestamp_ns,SwapCached_kB\n")
            
            # Write data rows
            for row in data:
                f.write(f"{row['timestamp_ns']},{row['SwapCached_kB']}\n")
...
def main():
    if len(sys.argv) != 3:
        print("Usage: python3 swapcached_timestamped.py <input_directory> <output_directory>", file=sys.stderr)
        print("Example: python3 swapcached_timestamped.py /path/to/data /path/to/output_directory", file=sys.stderr)
        return 1
    ...
    rawdata_name = "swapcached_timestamped"
