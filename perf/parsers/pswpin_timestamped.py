...
def parse_timestamped_file(input_file, output_file):
    """
    Parse timestamped pswpin data file and convert to CSV format with incremental values.
    ...
    try:
        with open(output_file, 'w') as f:
            # Write CSV header
            f.write("timestamp_ns,pswpin_pages\n")
...
def main():
    if len(sys.argv) != 3:
        print("Usage: python3 pswpin_timestamped.py <input_directory> <output_directory>", file=sys.stderr)
        print("Example: python3 pswpin_timestamped.py /path/to/data /path/to/output_directory", file=sys.stderr)
        return 1
    ...
    rawdata_name = "pswpin_timestamped"
