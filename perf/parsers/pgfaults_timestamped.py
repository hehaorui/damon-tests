...
def parse_timestamped_file(input_file, output_file):
    """
    Parse timestamped pgfault data file and convert to CSV format with incremental values.
    ...
    try:
        with open(input_file, 'r') as f:
            for line in f:
                ...
                # Parse line format: [timestamp_ns] pgfault: value
                ...
                if match:
                    ...
                    if incremental_value is not None:
                        data.append({
                            ...
                            'pgfaults_count': str(incremental_value)
                        })
    ...
    try:
        with open(output_file, 'w') as f:
            # Write CSV header
            f.write("timestamp_ns,pgfaults_count\n")
            
            # Write data rows
            for row in data:
                f.write(f"{row['timestamp_ns']},{row['pgfaults_count']}\n")
...
def main():
    if len(sys.argv) != 3:
        print("Usage: python3 pgfaults_timestamped.py <input_directory> <output_directory>", file=sys.stderr)
        print("Example: python3 pgfaults_timestamped.py /path/to/data /path/to/output_directory", file=sys.stderr)
        return 1
    ...
    rawdata_name = "pgfaults_timestamped"
