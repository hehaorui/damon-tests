...
def parse_timestamped_file(input_file, output_file):
    """
    Parse timestamped memory data file and convert to CSV format.
    ...
    try:
        with open(output_file, 'w') as f:
            # Write CSV header
            f.write("timestamp_ns,nr_free_pages\n")
...
def main():
    if len(sys.argv) != 3:
        print("Usage: python3 nr_free_pages_timestamped.py <input_directory> <output_directory>", file=sys.stderr)
        print("Example: python3 nr_free_pages_timestamped.py /path/to/data /path/to/output_directory", file=sys.stderr)
        return 1
    ...
    rawdata_name = "nr_free_pages_timestamped"
