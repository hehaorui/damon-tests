...
# Merge script wrapper for _do_merge.py
# Usage: CFG=custom_config.sh ./merge.sh

set -e  # Exit on any error

# Get the directory where this script is located
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
OUTPUT_DIR="$SCRIPT_DIR/merge_results"  # Output to the merge_results directory
...
# Prepare the command to run _do_merge.py
MERGE_SCRIPT="$SCRIPT_DIR/_do_merge.py"
...
python3 "$MERGE_SCRIPT" "$OUTPUT_DIR" "${unique_input_dirs[@]}"
